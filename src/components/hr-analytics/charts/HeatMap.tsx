...
function getCellClass(value: number) {
  if (value >= 85) return "bg-[#4f8bd6] text-white";
  if (value >= 70) return "bg-[#e56a8a] text-white";
  if (value >= 55) return "bg-[#f1b04a] text-[#111827]";
  return "bg-[#f2f8ff] text-[#4f8bd6]";
}
      ...
      >
        ...
        {data.map((row) => (
          <React.Fragment key={row.label}>
            <div className="flex items-center pr-3 text-xs font-medium text-[#111827]">
              {row.label}
            </div>
